...
void stage1()
{
    ...
    init (&per,&camera,&positionFond,&in,ecran,&level,&butn,&tab,&ps,&ind,&door,&mirro,&buck,&key,&pl,&lum,&butn1,&lo);
    while (continuer!=0)
    {
        start=SDL_GetTicks();
        ...
        lock(&per,ecran,level,camera,positionFond,&lo);
...
void init (perso *per,SDL_Rect *camera,SDL_Rect *positionFond,inpu *in,SDL_Surface *ecran,background *level,button *butn,tableau *tab,pause *ps,indices *ind,doors *door,mirror *mirro,bucket *buck,keys *key,plant *pl,lumiere *lum,button *butn1,locks *lo)
{
    int i,j;
    char im[50];
    level->cpt=0;
...
int lock (perso *per,SDL_Surface *ecran,background level,SDL_Rect camera,SDL_Rect positionFond,locks *lo)
{
